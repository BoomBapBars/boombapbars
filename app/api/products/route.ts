    ...
    products(first: $first) {
      edges {
        node {
          ...
          variants(first: 1) {
            edges { node { id title availableForSale price: priceV2 { amount currencyCode } } }
          }
        }
      }
    }
  ...
  try {
    if (!SHOPIFY_STORE_DOMAIN || !SHOPIFY_STOREFRONT_TOKEN) {
      return NextResponse.json(
        { error: 'Missing Shopify env vars', details: ['SHOPIFY_STORE_DOMAIN','SHOPIFY_STOREFRONT_TOKEN'] },
        { status: 500 }
      );
    }
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: `Shopify ${res.status}`, details: text }, { status: 502 });
    }
    ...
    const products = (json?.data?.products?.edges ?? []).map((e: any) => {
      ...
      return {
        ...
        tags: n.tags ?? [],
        images: img ? [img] : [],
        variants: v ? [{ id: v.id, title: v.title, availableForSale: v.availableForSale, price: v.price?.amount }] : [],
      };
    });
    ...
    return NextResponse.json(
      { error: 'Unhandled /api/products failure', details: err?.message ?? String(err) },
      { status: 500 }
    );
  }
}
